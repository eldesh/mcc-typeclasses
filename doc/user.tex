...
\newcommand{\version}{0.9.11}

\title{Münster Curry \\ User's Guide \\ {\normalsize Release \version}}
\author{Wolfgang Lux}
...
Precompiled binary distributions are available for Linux (i386), Mac OS X, and eventually other systems. After unpacking the archive, change into the directory corresponding to your platform and run the configure script in that directory. You can then use the scripts\footnote{The scripts \texttt{cyc}, \texttt{cymake}, and \texttt{cyi} are in fact soft links to \texttt{cyc-\version}, \texttt{cymake-\version}, and \texttt{cyi-\version}, respectively, allowing multiple versions of the Münster Curry compiler to coexist on a single machine.} \verb"cyc", \verb"cymake", and \verb"cyi" from the \verb"bin" directory (e.g., by setting up soft links from a directory which is included in \verb"$PATH") or install the compiler with \verb"make install". It is possible to uninstall the compiler with the command \verb"make uninstall". By default, the compiler binaries are installed in the directory \verb"/usr/local/bin", interface files, header files, and libraries in \texttt{/usr/local/lib/curry-\version}, and manual pages in \verb"/usr/local/man/man1".

The configure script is a standard Gnu configure script that understands the following options:
\begin{description}
  \item[\texttt{--prefix=}\emph{prefix}] Use \emph{prefix} instead of \verb"/usr/local" as base directory for the installation. For instance, in order to install the compiler in the directories \verb"bin" and \texttt{lib/curry-\version} in your home directory, you can use the command \verb"configure --prefix=$HOME".
  \item[\texttt{--bindir=}\emph{bindir}] Install the compiler binaries in directory \emph{bindir} instead of \verb"/usr/local/bin".
  \item[\texttt{--libdir=}\emph{libdir}] Install the library files in directory \emph{libdir}\texttt{/curry-\version} instead of \texttt{/usr/local/lib/curry-\version}.
...
make
\end{verbatim}
After successful compilation, you can use the scripts\footnote{The scripts \texttt{cyc}, \texttt{cymake}, and \texttt{cyi} are in fact soft links to \texttt{cyc-\version}, \texttt{cymake-\version}, and \texttt{cyi-\version}, respectively, allowing multiple versions of the Münster Curry compiler to coexist on a single machine.} \verb"cyc", \verb"cymake", and \verb"cyi" from the build directory (e.g., by setting up soft links from a directory which is included in \verb"$PATH") or install the compiler with the command \verb"make install". An installed compiler can be uninstalled with the command \verb"make uninstall".
...
If the compiler is configured with \texttt{--enable-trampoline}, it uses a portable but slower implementation where a global loop (the so-called trampoline) is used for dispatching tail calls. If your Curry programs keep crashing with bus errors or segmentation faults try reconfiguring the compiler with this option. Note that \texttt{--enable-trampoline} is the default when using a C compiler other than Gnu C and also on Mac OS X.
  \item[\texttt{--disable-pointer-tags}] Normally, the runtime system encodes character values and integer values that fit into $n-1$ bits, where $n$ is the number of bits in a machine word, in pointers rather than allocating a node in the heap for them. This improves efficiency mostly due to avoiding allocation, but makes pattern matching and character and integer operations slightly more complex. When the system is configured with \texttt{--disable-pointer-tags}, all characters and integer numbers are allocated in the heap. This option is useful only for measuring the performance gain of the default configuration and is not recommended otherwise.
...
On Mac OS X, if you have \texttt{TeXShop}\footnote{\url{http://darkwing.uoregon.edu/~koch/texshop/texshop.html}} installed, you can use it in order to typeset the documentation by using \verb"make TeXShop" instead of \verb"make curry.pdf". However, in contrast to the standard targets, the \verb"TeXShop" target does not yet take care of invoking \verb"bibtex" when necessary or typesetting the document once again if the cross references were updated.

\subsubsection{Building universal libraries and programs on Mac OS X}\label{sec:source-universal}
It is easily possible to build the Münster Curry compiler with universal libraries that can be used on both Intel and PowerPC based Macintosh computers by passing appropriate \verb"-arch" options to the C compiler either during configuration or while building the compiler. For instance, the command
\begin{verbatim}
  ./configure CFLAGS='-arch i386 -arch x86_64 -arch ppc -g -O2'
\end{verbatim}
will configure the Münster Curry compiler such that it can be used to link native executables on Intel based machines in 32-bit and 64-bit modes as well as on 32-bit PowerPC machines. It is also possible to build universal programs by specifying the appropriate \verb"-arch" options during the link step (see Sect.~\ref{sec:overall}).
...
\item Unpack the sources of the Münster Curry compiler in some convenient place and change into the \texttt{curry-\version} directory.
...
make install
\end{verbatim}
The last step may be omitted and the compiler can be used from its build directory, too. Note that at present, the networking related functions from modules \verb"IOExts" and \verb"Socket" do not work with MinGW.
...
\begin{description}
  ...
  \item[-q] This option has no effect.
...
If \verb"-o" is not specified, the default is to put an executable file in \verb"a.out", the object file for \emph{source.suffix} in \emph{source}\verb".o", its assembler file in \emph{source}\verb".s", and its C file in \emph{source}\verb".c".
  \item[-arch \emph{arch}] Mac OS X: Compile for the specified target architecture \emph{arch}. Multiple options work, and direct the compiler to produce ``universal'' binaries. Using this option on other systems is likely to produce a C compiler error.
\end{description}
...
Note that code compiled with this option is incompatible with code compiled without this option. This option must therefore be used consistently for compiling and linking all modules of a program.
  ...
  \item[-i\emph{dir}] Add directory \emph{dir} to the list of paths used to locate interface files. By default, the compiler searches for interface files in the current directory and in its standard library directory (usually \texttt{/usr/local/lib/curry-\version}).

The directory \emph{dir} is also added to the list of paths used to locate header files, as if \verb"-I"\emph{dir} was present on the command line.
  \item[-P\emph{dir}] Add directory \emph{dir} to the list of paths used to locate interface files. By default, the compiler searches for interface files in the current directory and in its standard library directory (usually \texttt{/usr/local/lib/curry-\version}) .

The directory \emph{dir} is also added to the lists of paths used to locate header and library
files, respectively, as if \verb"-I"\emph{dir} and \verb"-L"\emph{dir} were present on the command line.
  \item[-Whaskell]
...
\begin{description}
  ...
  \item[-I\emph{dir}] Add directory \emph{dir} to the list of paths used to locate header files. The standard library directory (usually \texttt{/usr/local/lib/curry-\version}) is always included in this list.
  \item[-F\emph{dir}] Mac OS X: Add the framework directory \emph{dir} to the list of paths used to locate header files. Using this option on other systems is likely to produce a C compiler error.
  ...
  \item[-\relax-ccopts \emph{opts}] Passes each option from the comma separated list \emph{opts} to the C compiler without further interpretation.
\end{description}
...
\begin{description}
  ...
  \item[-f] Print all failures during execution of a non-deterministic goal. If given multiple times, print also failures inside encapsulated search.
  ...
  \item[-e\emph{goal}] Evaluate \emph{goal} instead of \verb"main". See Sect.~\ref{sec:goals} for a description of the goal syntax. The \emph{goal} is evaluated with respect to the modules specified with \verb"-M" options and the Curry source files appearing on the command line. All
entities exported from the \verb"Prelude" and the Curry source file that appears last on the command line or the last module specified with a \verb"-M" option, if there are no source file arguments, are in scope with unqualified and qualified names. The exported entities of all other
modules are in scope with qualified names only.
  \item[-T\emph{goal}] Compute the type of \emph{goal}. See Sect.~\ref{sec:goals} for a description of the goal syntax. The \emph{goal}'s type is computed with respect to the modules specified with \verb"-M" options and the Curry source files appearing on the command line. All
entities exported from the \verb"Prelude" and the Curry source file that appears last on the command line or the last module specified with a \verb"-M" option, if there are no source file arguments, are in scope with unqualified and qualified names. The exported entities of all other
modules are in scope with qualified names only.
  \item[-M\emph{module}] Brings all entities exported from \emph{module} into scope with their qualified names for a goal specified with the \verb"-e" and \verb"-T" options. By default, only the entities exported from the \verb"Prelude" and the Curry source files occurring on the command line are in scope. The entities exported from the \verb"Prelude" and the last source file appearing on the command line or specified with the last \verb"-M" option, if there are no source file arguments, are brought into scope with their unqualified names, too.

  \emph{Module} is either the name of a source file (ending in  \verb".lcurry" or \verb".curry"), the name of an interface file (ending in \verb".icurry"), or the name of a module whose interface can be located in the interface search path.
  \item[-L\emph{dir}] Add directory \emph{dir} to the list of paths used to locate libraries specified with \verb"-l" options. The standard library directory (in the standard configuration: \texttt{/usr/local/lib/curry-\version}) is always included in this list.
  \item[-l\emph{lib}] Search the library named \emph{lib} when linking.
  \item[-framework \emph{name}] Mac OS X: Link against framework \emph{name}. Using this option on other systems is likely to produce a C compiler error.
  ...
  \item[-\relax-ldopts \emph{opts}] Passes each option from the comma separated list \emph{opts} to the C compiler without further interpretation. As with \verb"--ldopt", \emph{opts} are used only during the link phase.
\end{description}

\subsection{Environment}
\begin{description}
\item[CURRY\_PATH] The environment variable \verb"CURRY_PATH" is used to locate the compiler executables. The default value is \texttt{/usr/local/lib/curry-\version}.
...
\begin{description}
  ...
  \item[-\relax-find] For each command line target $t$, \verb"cymake" will print a line of the form $t$\verb": "$f$, where $f$ is the name of the source or interface file corresponding to $t$. If $t$ is a module name, \verb"cymake" will look for the source or interface file first in the current directory and then along the search path. If no file is found, $f$ is empty.
  ...
  The directory \emph{dir} is also added to the list of paths used to locate C header files, as
if \verb"-I"\emph{dir} was present on the command line.
  \item[-P\emph{dir}] Add directory \emph{dir} to the list of paths used to locate interface files for library modules. By default, the compiler searches for interface files in its standard library directory (usually \texttt{/usr/local/lib/curry-\version}).
  
  The directory \emph{dir} is also added to the lists of paths used to locate C header files and
libraries, respectively, as if \verb"-I"\emph{dir} and \verb"-L"\emph{dir} were present on the command line.
...
\subsection{Environment}
\begin{description}
\item[CURRY\_PATH] The environment variable \verb"CURRY_PATH" is used to locate the auxiliary executables used by \verb"cymake".  The default value is \texttt{/usr/local/lib/curry-\version}.
\item[CURRY\_IMPORT\_PATH] 
\item[CURRY\_INCLUDE\_PATH] \vspace{-\medskipamount}
...
A goal is a Curry expression optionally followed by a where-clause, which can be used for providing local definitions for the goal (see appendix C.3 in~\cite{Hanus06:Report} for the context free syntax of Curry).
\begin{eqnarray*}
\emph{Goal} & ::= & \mbox{\emph{Expr} [\texttt{where} \emph{LocalDefs}]}
\end{eqnarray*}

All entities exported from the \verb"Prelude" and the module specified in the last \verb":load"
command are in scope with unqualified and qualified names unless they are shadowed by one of the declarations from the where-clause. Entities from modules that are imported directly or indirectly from the module specified in the last \verb":load" command are also in scope, but only with qualified names.

All free variables of the goal must be declared either in the where-clause or by using a let expression as goal. The bindings of the free variables are displayed together with the normal form of the goal after successful evaluation unless the goal is of type \verb"IO"~$t$.
\subsection{Commands}
...
\begin{description}
  ...
  \item[:load \emph{module}] Set the current evaluation context to \emph{module}. This command will compile \emph{module} and all modules it depends on if necessary. \emph{Module} can be specified either by the name of its source file (ending in \verb".lcurry" or \verb".curry") or a plain module name. In the latter case, \verb"cyi" will look for a source or interface file for \emph{module} in the current directory and the current search path as determined by the \verb"-i" and \verb"-P" options set with the \verb":set" command and the environment variable \verb"CURRY_IMPORT_PATH".

  \verb"cyi" displays the name of the current module in its prompt.
  ...
  \item[:interface \emph{module}] Display the interface of \emph{module}. If the environment variable \verb"PAGER" is set, the interface file is displayed with this command. Otherwise the interface is simply written to the standard output.
  \item[:interface] Display the interface of the current module.
...
\verb"  / /     | / | |              Version 0.9.11, Copyright (c) 1998-2007"
\verb" / /___  / /  | |"
...
\subsubsection{Fixity Declarations}\label{sec:ext-fixity}
...
\end{verbatim}
Note that the operator \verb"<==" is also defined in library module \texttt{Success} (Sect.~\ref{sec:lib-Success}).

For compatibility with Haskell, the precedence level in fixity declarations may be omitted, in which case it defaults to 9. Thus, the declarations \verb"infixr 9 ." and \verb"infixr ." are equivalent.
...
\subsubsection{No Automatic Eta-expansion of Function Definitions}\label{sec:ext-eta}
Similar to PAKCS version 1.8 and \Toy{}, the arity of a function is determined by the number of arguments in its declaration rather than by its type as the Curry report implies. For instance, the declaration \verb"sum = foldr (+) 0" defines \verb"sum" as a nullary function even though it has type \texttt{[Int] -> Int}.

Note that $\eta$-expansion can change the semantics of a program. Consider the two programs
\begin{center}\tt
  \begin{tabular}{ll}
    ...
    \quad where x,y free & \quad where x,y free \\
  \end{tabular}
\end{center}
For the left program, \verb"goal fcoin" has just two solutions, namely \verb"0" and \verb"2", whereas for the right program, which is the $\eta$-expanded version of the left program, \verb"goal fcoin" has three different solutions, namely \verb"0", \verb"1", and \verb"2".

Nevertheless, the compiler performs $\eta$-expansion as an optimization when it can prove that the semantics of the program remains unaffected. In particular, function definitions with only a single equation and whose right hand side is a non-expansive expression (cf.\ Sect.~\ref{sec:poly-let}) are always $\eta$-expanded.
...
\subsubsection{Polymorphic Generalization of Let-Bound Variables}\label{sec:poly-let}
...
\item applications of a constructor with arity $n$ to at most $n$ non-expansive argument expressions,
\item applications of a function or $\lambda$-expression with arity $n$ to at most $n-1$ non-expansive argument expressions, and
\item let-expressions which comprise only function declarations and variable declarations of the form $x=e$ where $e$ is a non-expansive expression and whose body is also a non-expansive expression.
\end{itemize}
...
\end{verbatim}
where the types $\forall\alpha. [\alpha]$, $\forall\alpha. \alpha\rightarrow[\texttt{Int}]$, and $\forall\alpha. [\alpha]\rightarrow\alpha$ are inferred for the local variables \verb"nil", \verb"z", and \verb"last", respectively. On the other hand, the definition
\begin{verbatim}
f4 = (1:nil, 'a':nil) where nil = id []
\end{verbatim}
is still rejected because \verb"id []" is an expansive expression and therefore \verb"nil"'s type is not generalized.
...
\mbox{\meta{CallConv}} & ::= & \literal{primitive} \mid \literal{ccall} \mid \literal{rawcall} \\
...
\subsection*{Release 0.9.11 (June 10\textsuperscript{th}, 2007)}
\paragraph{Build Environment}
  \begin{itemize}
  \item On Microsoft Windows, the compiler can now also be built with MinGW.
  \item Versioned installation will allow multiple versions of the Münster Curry compiler to coexist on a single machine in the future.
  \item New make target \verb"TeXShop" for typesetting the documentation on Mac OS X with \texttt{TeXShop.app} (cf.\ Sect.~\ref{sec:source-documentation}).
  \item It is now possible to build universal libraries on Mac OS X (cf.\ Sect.~\ref{sec:source-universal}).
  \end{itemize}
\paragraph{Compiler}
  \begin{itemize}
  \item Function definitions are no longer $\eta$-expanded automatically (cf.\ Sect.~\ref{sec:ext-eta}).
  \item Let-bound variables can have a polymorphic type in some cases (cf.\ Sect.~\ref{sec:poly-let}).
  \item The precedence level in a fixity declaration is now optional and defaults to 9 if omitted (cf.\ Sect.~\ref{sec:ext-fixity}).
  \item Only the exported entities of the main module and the Prelude are visible with unqualified names in a goal. On the other hand, the entities of all other modules imported directly or indirectly from the main module can now be accessed with qualified names.
  \item Code generation is now based on a register-based abstract machine code model, which yields more efficient code.
  \item The compiler omits redundant module qualifiers from type expressions in error messages.
  \item Limited support for compiling mutually recursive modules.
  \item Compiler options \verb"--ccopts" and \verb"--ldopts" allow passing multiple comma separated options to the C compiler (cf.\ Sects.~\ref{sec:compiling-c} and~\ref{sec:linking}).
  \item Compiler accepts options \verb"-F" and \verb"-framework" and passes them on to the C compiler.
  \end{itemize}
\paragraph{Debugger}
  \begin{itemize}
  \item Debugging of goals involving \texttt{IO} computations now works.
  \end{itemize}
\paragraph{Interactive Environment}
  \begin{itemize}
  \item The interpreter's \verb":load" command takes changes of the search path with the \verb":set" command into account.
  \item The interpreter allows \verb":load"ing standard library interfaces so that, e.g., \verb":l Array" no longer fails with an error message \texttt{missing source file for Array}.
  \item The interpreter no longer recompiles source modules each time a goal is evaluated.
  \item Only the exported entities of the main module and the Prelude are visible with unqualified names in a goal. On the other hand, the entities of all other modules imported directly or indirectly from the current module can now be accessed with qualified names (cf.\ Sect.~\ref{sec:goals}).
  \item New command \verb":interface" that displays the interface of the current or a specified module.
  \end{itemize}
\paragraph{Curry Library and Runtime System}
  \begin{itemize}
  \item Improved trampoline implementation (used by default on Mac OS X), which reduces execution times by 5 to 10\% on a PowerPC, and up to 30\% on x86.
  \item Expressions like \verb"chr (-1)" and \verb"const 0 <= id" now fail only the current solution instead of aborting the program with an error message.
  \item When a program fails due to a pattern match failure, the error message now includes the function where the failure was detected.
  \item Pattern matching (and other) failures are reported in non-IO goals if the goal was compiled with the \verb"-f" option (cf.\ Sect.~\ref{sec:linking}) or if the program is invoked with the \verb"-f" runtime system option (cf.\ Sect.~\ref{sec:running}).
  \item The runtime system now uses tagged characters (similar to tagged integers) by default. This avoids allocation for characters with codes above 255 (decimal).
  \item Avoid many name conflicts when interfacing with foreign C code.
  \item Many portability fixes to allow compilation on Windows with MinGW.
  \end{itemize}
\paragraph{Foreign Function Interface}
  \begin{itemize}
  \item New non-standard calling convention \texttt{rawcall} (cf.\ Sect.~\ref{sec:ext-ffi}).
  \end{itemize}
\paragraph{Noteworthy Bug Fixes}
  \begin{itemize}
  \item The compiler no longer fails with an internal error in function \verb"constrKind" when importing a module whose interface contains types $T_{1}$ and $T_{2}$ whose names differ only by their module qualifiers.
  \item The compiler no longer fails with a non-exhaustive match when reporting case mode warnings for a program with explicit type signatures.
  \item Setting the initial buffering mode for the standard input and output channels with the \verb"-b" runtime system option (cf.\ Sect.~\ref{sec:running}) now works.
  \item The compiler no longer infers too general types for some functions in mutually recursive binding groups involving pattern declarations.
  \item The parser no longer rejects patterns of the form $(\emph{op})\,t_{1}\dots t_{n}$ when \emph{op} is a qualified operator, e.g., \verb"(Assoc.:=) x y".
  \end{itemize}
